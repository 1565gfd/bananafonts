...
| Версия | Что |
|--------|-----|
| v5.23.x | **Инструменты перемещены в Настройки** — вкладка `tools` удалена, 4 секции (Password gen / Random number / Lorem Ipsum / Color converter) теперь под Sound toggle в Settings. Удалены ссылки `tabBtnTools`/`toolsTitleEl` + локали `tabTools`/`toolsTitle`. Описание сайта обновлено. |
