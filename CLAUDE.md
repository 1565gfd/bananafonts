...
| Версия | Что |
|--------|-----|
| v5.21.x | **+2 советские темы**: `soviet-lift` (faded gray-green painted metal) и `soviet-podyezd` (dim entrance hall с single yellow bulb radial). **Радужная переименована** → «Переливающаяся» (ru) / «Iridescent» (en). Кнопка перенесена в **последнюю позицию** на main-bar — должна оставаться последней при добавлении новых. `data-theme-btn="rainbow"` сохранён для backward-compat (localStorage, CSS, animation keyframes). **Subtract-handler defensive**: `getAttribute` fallback на случай если `dataset.addSec` undefined. Итого тем: **17**. |
