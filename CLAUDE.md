...
| Версия | Что |
|--------|-----|
| v5.32.x | **Liquid Glass redesign** в стиле iOS 26: tabs → pill-контейнер с pill-кнопками (active-state с accent fill + inner-glow shadow); все кнопки (tool-btn, settings-btn, theme/lang switches, calc-btn) — `backdrop-filter: blur(20-28px) saturate(180-200%)` + `::before` edge-light gradient pseudo + многослойные inset box-shadows; `.app` контейнер `border-radius: 32px`, blur 40px, ::before refraction overlay; tool/settings/time секции — glass-карточки 22px радиус. Inputs/textareas: blur+saturate + focus-glow. **+ Анти-троллинг** (v5.31): SWEAR_PATTERNS (~80 паттернов, 9 языков + Гитлер/нацисты), red lockout с 5-сек countdown, 3-strikes wipe, 3-trigger session limit, auto-accept SORRY/ПРОСТИ. |
| v5.30.x | **Админ-панель расширена** — +5 новых секций: **Тест звуков** (12 кнопок, по одной на каждый sound type), **Effects** (banana rain / heart / matrix / flip / shake / noir / sweep / big-67), **Custom emoji rain** (emoji input + count 1-200), **localStorage inspector** (таблица всех `bananafont:*` ключей с inline delete-кнопками), **Export/Import settings** (JSON dump/restore через `<a download>` и `FileReader`, импорт реаплаит тему). |
| v5.29.x | **Админ-панель** для 1565gfd — модальное окно с три секции: stats (version/theme/lang/sound/volume/storage entries), all-themes grid (one-click apply включая секретные), public codes list (без приватного). Активируется **отдельным приватным кодом** который хранится как XOR-encoded байтовый массив (`_ax`/`_ad`/`_isPrivilegedCode` в `js/app.js`). Plaintext этого кода **намеренно отсутствует** в репозитории — только в чат-сессии которая создала фичу. **v5.29.1**: критичный CSS hidden-bug фикс (`display: flex` перебивал `[hidden]` → панель открывалась при старте и не закрывалась). |
...
| v5.21.x | **+2 советские темы**: `soviet-lift` (walnut wood + warm ceiling spotlights, v5.21.2 redesigned to match Otis cabin photo) и `soviet-podyezd` (white-top/teal-bottom split painted walls, v5.21.2 redesigned to match panel-house entrance photo). **Радужная переименована** → «Переливающаяся» (ru) / «Iridescent» (en). Кнопка перенесена в **последнюю позицию** на main-bar — должна оставаться последней при добавлении новых. `data-theme-btn="rainbow"` сохранён для backward-compat (localStorage, CSS, animation keyframes). **Subtract-handler defensive**: `getAttribute` fallback. Итого тем: **17**. |
| v5.20.x | **+4 уникальные темы** в Settings → итого 9 дополнительных + 5 основных + школа = **15 тем**. Новые: `cyberpunk` (magenta+cyan), `coffee` (warm browns), `terminal` (CRT phosphor green-on-black + glow text-shadow), `aurora` (animated green→cyan→violet, 16s — reuses rainbowFlow keyframe). **Reset warning box** — красная danger-zone в Settings: явное предупреждение о том что будет удалено + danger-styled кнопка. `settings-danger-zone`, `settings-warning-box`, `settings-btn-danger` CSS classes. |
| v5.19.x | **6 новых тем**: `black-orange` в главную панель (5-я кнопка), + 5 дополнительных в Settings («Дополнительные темы» с swatch-grid): `neon-green`, `neon-violet`, `pastel`, `ocean`, `sunset`. Каждая тема — полный CSS variables block + mobile fallback. `EXTRA_THEMES` массив в JS, `rebuildExtraThemesGrid()` рендерит и подсвечивает активную. Все 11 тем теперь в `VALID_THEMES`, `THEME_COLORS`, `THEME_LABEL_KEYS`, VALID arrays обоих HTML. **UI polish**: Settings sections получили accent-bar `::before`, Secret-codes input — focus glow, Fonts grid — smoother hover lift. |
| v5.18.x | **Кнопки отнять время** в таймере: `−30s / −1m / −5m` второй строкой (с лёгким красным контуром). `timerAddSeconds(addSec)` теперь принимает negatives. Underflow: running → `timerFinish()` сразу; paused/idle → clamp в 0; done с negative → ignore. Toast «Убрано: −1m» / «Removed: −1m». |
| v5.17.x | **Smart word counter** на главной: `isRealWord(token)` — токен считается словом только если он 1-символьный (а/я/I/a) ИЛИ содержит ≥2 разных символа (case-insensitive). Фильтрует «вввв пппп» → 0 слов. **Silence-on-stop звука**: трекаем `pendingChimeTimeouts` + `pendingChimeNodes`, `silenceAllChimes()` отменяет scheduled repeats и фейдает в тишину живые osc'ы за 50ms (без щелчка). Вызывается из `timerReset` и `stopAlarmRinging` (Dismiss будильника). |
